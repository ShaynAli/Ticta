...
class TTTInterface:
    def __init__(self, shape_generator=shapes_gen, color_generator=colors_gen, game_rows=3, game_columns=3, players=2, gui_cell_size=140):
        '''
...
class GameGUI:
    ...
    def build_gui(self):
        ...
        for x in range(0, self.rows):
            for y in range(0, self.columns):
                ...
                button.bind('<Button-1>', self.draw)
                self.button_array[x][y] = button

        self.button_start.bind("<Button-1>", self.new_game)
    ...
    def draw(self, event):
        if event.widget["state"] == NORMAL:
            ...
            co_ordinates, color = self.game_grid_click(x, y)
